...
def train_device() -> torch.device:
    """Training device: CUDA (with LOCAL_RANK when using torchrun) or CPU."""
...
def create_dataloader(
    config: PretrainConfig, split: str, rank: int, world_size: int, device: Optional[torch.device] = None, **kwargs
):
...
def create_model(
    config: PretrainConfig, train_metadata: PuzzleDatasetMetadata, rank: int, world_size: int, device: Optional[torch.device] = None
):
    model_cfg = dict(
...
def init_train_state(
    ...
    load_checkpoint(train_state, config, rank, device=device)
...
def load_checkpoint(train_state: TrainState, config: PretrainConfig, rank: int, device: Optional[torch.device] = None):
    if device is None:
        device = train_device()
...
def launch(hydra_config: DictConfig):
    ...
    if "LOCAL_RANK" in os.environ:
        # NCCL requires CUDA; macOS / CPU builds use gloo.
        ...
        # CPU GLOO process group (evaluators / host-side collectives)
    ...
    if RANK == 0:
        ...
        try:
            ...
            _save_dir = str(HydraConfig.get().runtime.output_dir)
        except Exception:
            _save_dir = os.getcwd()
        save_code_and_config(config, _save_dir)
